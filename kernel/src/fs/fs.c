...
int fs_open(const char *pathname, int flags)
{
	...
	for (int i = 0; i < NR_FILES; ++i) {
        if (strcmp(pathname, file_table[i].name) == 0) {
            if (file_table[i].disk_offset >= sz) {
                continue;
            }
            files[i + 3].used = true;
            files[i + 3].index = i;
